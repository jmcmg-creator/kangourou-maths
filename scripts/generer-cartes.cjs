...
require('fs').writeFileSync(require('path').join(require('fs').mkdtempSync(require('path').join(require('os').tmpdir(),'cartes-')),'apercu.html'),`<body style="margin:0;background:#111;display:flex;gap:8px;flex-wrap:wrap">${['asie','afrique','amerique'].map(svg).join('')}</body>`);
